...
(records as TestRecord[]).forEach((record, index) => {
  test(`Row ${index + 1}: #C-Reactive-borrow-staging-happypath - ${record.firstName || 'User'}`, async ({ page }) => {
    ...
    await page.getByPlaceholder('MM/YYYY').fill(record.moveInDate);
    await page.waitForTimeout(2000);
    await page.locator('button').filter({ hasText: 'Next' }).click();
    ...
    await page.getByRole('spinbutton', { name: 'How much is your share of the' }).click();
    await page.getByRole('spinbutton', { name: 'How much is your share of the' }).press('ControlOrMeta+z');
    await page.getByRole('spinbutton', { name: 'How much is your share of the' }).fill('100');
    await page.locator('button').filter({ hasText: 'Next' }).click();
