...
test('happy-asda', async ({ page }) => {
  ...
  const testData: any = {
    ...
    inputs: {
      ...
      jobTitle: 'TheBoss',
      employmentStartDate: '01/1985',
      grossAnnualIncome: '10000',
      netMonthlyIncome: '998',
      otherIncome: '200',
  ...
  await page.getByRole('spinbutton', { name: '47,000' }).click();
  await page.getByPlaceholder('47,000').fill(testData.inputs.loanAmount);
  ...
  await page.getByRole('heading', { name: 'Your Income From KingstonNHS' }).click();
  await page.locator('#input-295').fill(testData.inputs.grossAnnualIncome);
  await page.locator('#input-299').fill(testData.inputs.netMonthlyIncome);
  await page.locator('#input-305').fill(testData.inputs.otherIncome);
  await page.waitForTimeout(3000);
